using BMS.Shop.IdentitySettings;
using System;
...
namespace BMS.Shop.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(ShopConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(ShopConsts.ProductIdentitySettingId, "Sản phẩm", ShopConsts.ProductIdentitySettingPrefix, 1, 1));
